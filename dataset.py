...
def _load_dataset(name, img_id2val, dataroot='/datasets/ee285f-public/VQA2017'):
    """Load entries
    ...
    answer_path = os.path.join('./data', 'cache', '%s_target.pkl' % name)
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='./data'):
        ...
        print('loading features from imgid2idx pickle file')
        ...
        self.img_id2idx = dict(img_id2idx_list[:L//100])
        ...
        with h5py.File(h5_path, 'r') as hf:
            ...
            self.features = np.array(features[:features.shape[0]//100])
            spatials = hf.get('spatial_features')
            self.spatials = np.array(spatials[:spatials.shape[0]//100])
